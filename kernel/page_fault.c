...
static void print_console_status(uint32_t error_code)
{
...
static void serial_write_status(uint32_t error_code)
{
...
struct registers *page_fault_handler(struct registers *regs)
{
    ...
    console_write("\nERROR:     0x");
    console_write_hex32(regs->err_code);
    console_write("\n");

    print_console_status(regs->err_code);
    console_write("================================\n");

    serial_write("\n\n========== PAGE FAULT ==========\n");
    serial_write("CR2:       ");
    serial_write_hex32(cr2);
    serial_write("\nEIP:       ");
    ...
    serial_write_status(regs->err_code);
    serial_write("================================\n");

    if (((regs->cs & 0x3U) == 0x3U) || ((regs->err_code & 0x4U) != 0U)) {
        console_set_color(VGA_LRED, VGA_BLACK);
        console_write("User mode task fault detected. Terminating task.\n");
        serial_write("User mode task fault detected. Terminating task.\n");
    ...
    console_write("Kernel page fault is fatal. CPU halted.\n");
    serial_write("Kernel page fault is fatal. CPU halted.\n");
